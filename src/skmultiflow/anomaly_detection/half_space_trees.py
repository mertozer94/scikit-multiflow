...
class HalfSpaceTrees(BaseSKMObject, ClassifierMixin):
    ...
    def initialise_work_space(self):
        ...
        for i in range(self.dimensions):
            ...
            try:
                self.min[i] = min_element
            except IndexError:
                self.min.append(min_element)
            try:
                self.max[i] = max_element
            except IndexError:
                self.max.append(max_element)
    ...
    def build_trees(self):
        ...
        for i in range(self.n_estimators):
            ...
            tree = HalfSpaceTree(self.depth, self.dimensions, self.size_limit)
            tree.root = tree.build_tree(self.min, self.max)
...
class HalfSpaceTree:
    ...
    def __init__(self, max_depth, dimensions, size_limit):
        """
        Half Space Tree
    ...
    def _anomaly_score(self, node, X):
        ...
        if node is not None:
            if node.internal_node is not True or node.r < self.size_limit:
                anomaly_score = node.r * pow(2.0, node.depth)
            else:
                if X[node.split_attribute] > node.split_value:
                    anomaly_score = self._anomaly_score(node.right, X)
                else:
                    anomaly_score = self._anomaly_score(node.left, X)

        return anomaly_score

    def build_tree(self, min, max, current_depth=0):
        """
        ...
        if self.max_depth == current_depth:
            ...
            random_dimension = random.randint(0, self.dimensions - 1)
            p = (min[random_dimension] + max[random_dimension]) / 2.0
            temp = max[random_dimension]
            max[random_dimension] = p
            left = self.build_tree(min, max, current_depth + 1)
            max[random_dimension] = temp
            min[random_dimension] = p
            right = self.build_tree(min, max, current_depth + 1)
            return HalfSpaceTreeNode(left=left, right=right, split_attribute=random_dimension,
    ...
    def update_model(self, node):
        ...
        """
        if node is not None:
            if node.r is not 0 and node.l is not 0:
                node.r = node.l
